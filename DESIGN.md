...
- **Silence padding — the alignment invariant:** Discord only delivers RTP frames while a user is actively speaking, so a naive append would concatenate bursts with no representation of the silence between them. Each user's `.pcm` is therefore padded with zero bytes so that **byte offset `N` == session time `N / 192` ms** (192 = 48 kHz × 2 ch × 2 B). Transcription relies on this: it transcribes each user's WAV independently and merges by timestamp, so if the padding is wrong every speaker's audio slides toward `t=0` and the timeline compresses (distinct speakers appear to overlap; total duration shrinks). The pad before each frame is computed by the pure `planPad()` (`recorder/src/timeline.js`); how much to insert is driven by Discord's `speaking` start/end VAD events, with a wall-clock safety re-anchor for gaps Discord never closes.
  - **Backpressure is load-bearing here.** A multi-second gap is tens of MB of padding — more than a `WriteStream`'s high-water mark, so `write()` returns `false` mid-pad. The padding **must** be drained across `drain` events (pausing the decoder meanwhile), never dropped on the first backpressure signal. `recorder/src/pump.js` (`UserPcmWriter`) owns this; dropping the overflow is the historical regression that silently compressed sessions (e.g. 50 min → 18 min). Both `timeline.js` and `pump.js` are dependency-free and unit-tested under `recorder/test/`.
