...
async def transcribe_session(
    ...
    """Transcribe every <user_id>.wav under audio_dir, persisting per-speaker
    segments immediately so a crash mid-batch doesn't redo finished users.

    Driven by the WAV files actually on disk — not by `members` — so a speaker
    whose display name never made it into `members` (dropped IPC speaker event,
    spoke-and-left between heartbeats) is still transcribed instead of being
    silently dropped. `members` is consulted only as a {user_id: display_name}
    lookup, falling back to "User<id>".

    Segment timestamps are already on the session timeline because the recorder
    silence-pads each user's PCM so byte offset == session time. Returns a flat
    list of all segments, sorted by start time.
    """
    all_segments: list[Segment] = []
    for wav in sorted(audio_dir.glob("*.wav")):
        user_id = wav.stem
        display_name = members.get(user_id) or f"User{user_id}"
